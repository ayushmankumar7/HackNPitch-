...
while True:
    ...
    if counter > 60:
        for light in traffic_lights:
            ...
            canvas.create_oval(x,210,x+10,220, fill=color[traffic_lights[2]['status']])
        canvas.update()
        counter = 0
    ...
    if car[0] ==100:
        for light in traffic_lights:
            ...
            canvas.create_oval(x,210,x+10,220, fill=color[light['status']])
        canvas.update()
    if car1[0] >= 140 and traffic_lights[2]['status'] == 0:
        canvas.move(carimg2,traffic_lights[2]['xposition']-(car1[0]-speed),0)
        while True:
            counter += 1
            if counter > 60:
                ...
                flag = True
            if flag is True:
                canvas.move(carimg2,random.randint(0,5),0)    
            canvas.move(carimg,2,0)
            canvas.update()    
            time.sleep(sleeptimer)
